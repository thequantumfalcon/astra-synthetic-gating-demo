...
SNAPSHOT_DEPOSIT = (
    REPO_ROOT / "release_snapshot" / "release_deposit_2026-01-16_v5_final"
)
...
def _normalize_newlines(b: bytes) -> bytes:
    # release archive goldens were produced on Windows (CRLF). Normalize to LF for
...
def _extract_goldens(tmp: Path) -> Path:
    if not SNAPSHOT_ANCILLARY_ZIP.exists():
        raise SystemExit(
            f"Missing snapshot ancillary zip: {SNAPSHOT_ANCILLARY_ZIP} (did you copy release_snapshot?)"
        )
...
def main() -> None:
    ...
    # Windows environment used to generate the release archive goldens. To avoid CI

...
class LiquidPhysics:
    ...
    def __init__(self, config: dict):
        """Initialize the solver from the engine physics config."""
        ...
        self.g = jnp.array([0.0, -GRAVITY_MS2, 0.0], dtype=jnp.float32)

    @partial(jax.jit, static_argnums=(0,))
    def kernel_poly6(self, r: jnp.ndarray) -> jnp.ndarray:
        """Evaluate the Poly6 smoothing kernel for pairwise distances."""
        q = r / self.h
        coef = POLY6_COEFFICIENT / (64.0 * jnp.pi * (self.h**9))
    ...
    def kernel_spiky_grad(self, r_vec: jnp.ndarray, r_len: jnp.ndarray) -> jnp.ndarray:
        ...
        Returns: (N, N, 3)
        """
        r_len_safe = jnp.maximum(r_len, MIN_DISTANCE)
        coef = SPIKY_GRAD_COEFFICIENT / (jnp.pi * (self.h**6))
    ...
    def step(
        ...
        rho = jnp.maximum(rho, MIN_DISTANCE)

        pressure = self.rho0 * (self.c0**2 / PRESSURE_EXPONENT) * (
            (rho / self.rho0) ** PRESSURE_EXPONENT - 1.0
        )
        ...
        shift_vel = (
            TVF_SHIFT_SCALE
            * self.h
            * jnp.sum(((mass / rho)[None, :, None]) * grad_w, axis=1)
        )
        ...
        hit = (new_pos > DOMAIN_LIMIT) | (new_pos < -DOMAIN_LIMIT)
        new_vel = jnp.where(hit, -new_vel * BOUNDARY_DAMPING, new_vel)
        new_pos = jnp.clip(new_pos, -DOMAIN_LIMIT, DOMAIN_LIMIT)
    ...
    def export_step_fn(
        ...
        def step_fn(
            ...
            pressure = rho0 * (c0**2 / PRESSURE_EXPONENT) * (
                (rho / rho0) ** PRESSURE_EXPONENT - 1.0
            )

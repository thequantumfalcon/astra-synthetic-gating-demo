...
Treat `zenodo_deposit_2026-01-16_v5_final/` as **release evidence**: don’t edit its contents during development.
...
- `repro/`: portable scripts:
  ...
  - `repro/verify_astra.py` verifies outputs against goldens extracted from `zenodo_deposit_2026-01-16_v5_final/ASTRA_Ancillary_v5.zip`.
  - `repro/build_paper.py` builds the PDF from `astra_submission_bundle/`.
- `zenodo_deposit_2026-01-16_v5_final/`: immutable Zenodo deposit (goldens + audits + checksums).
...
- Keep snapshot integrity consistent:
  - Don’t edit `zenodo_deposit_2026-01-16_v5_final/` during normal development.
...
Avoid editing `zenodo_deposit_2026-01-16_v5_final/` except when intentionally producing a new release snapshot.
